...
export async function handleStatus(args: {
  zone?: 'all' | 'corona' | 'inner' | 'habitable' | 'outer' | 'kuiper' | 'oort';
  limit?: number;
...
export async function handleCommit(args: {
  ...
  try {
    ...
    const text = `✓ Committed | decisions: ${(args.decisions ?? []).length} | steps: ${(args.next_steps ?? []).length} | errors: ${(args.errors ?? []).length} | orbit changes: ${changes.length}`;
...
export async function handleRecall(args: {
  ...
  try {
    ...
    const results: Memory[] = await recallMemoriesAsync(proj, args.query, {
      type:        memoryType,
    ...
    const lines: string[] = [
      `Recall: "${args.query}" — ${results.length} result${results.length === 1 ? '' : 's'} (pulled closer to Sun)`,
      '',
    ];
...
export async function handleForget(args: {
  ...
  try {
    ...
    return {
      content: [{
        type: 'text' as const,
        text: `↑ Pushed ${args.id} to ${oortDistance.toFixed(2)} AU (${zoneLabel}) — still recoverable via recall.`,
      }],
    };
  } catch (err) {
    if (err instanceof McpError) throw err;
